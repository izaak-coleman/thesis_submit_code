...
void bwa_print_sam1(const bntseq_t *bns, bwa_seq_t *p, const bwa_seq_t *mate, int mode, int max_top2, FILE* fout)
{
	int j;
	if (p->type != BWA_TYPE_NO_MATCH || (mate && mate->type != BWA_TYPE_NO_MATCH)) {
		...
		err_fprintf(fout, "%s\t%d\t%s\t", p->name, flag, bns->anns[seqid].name);
		err_fprintf(fout, "%d\t%d\t", (int)(p->pos - bns->anns[seqid].offset + 1), p->mapQ);

		// print CIGAR
		if (p->cigar) {
			for (j = 0; j != p->n_cigar; ++j)
				err_fprintf(fout, "%d%c", __cigar_len(p->cigar[j]), "MIDS"[__cigar_op(p->cigar[j])]);
		} else if (p->type == BWA_TYPE_NO_MATCH) err_fprintf(fout,"*");
		else err_fprintf(fout,"%dM", p->len);
		...
		if (mate && mate->type != BWA_TYPE_NO_MATCH) {
			...
			err_fprintf(fout,"\t%s\t", (seqid == m_seqid)? "=" : bns->anns[m_seqid].name);
			isize = (seqid == m_seqid)? pos_5(mate) - pos_5(p) : 0;
			if (p->type == BWA_TYPE_NO_MATCH) isize = 0;
			err_fprintf(fout,"%d\t%lld\t", (int)(mate->pos - bns->anns[m_seqid].offset + 1), isize);
		} else if (mate) err_fprintf(fout,"\t=\t%d\t0\t", (int)(p->pos - bns->anns[seqid].offset + 1));
		else err_fprintf(fout,"\t*\t0\t0\t");

		// print sequence and quality
		if (p->strand == 0)
			for (j = 0; j != p->full_len; ++j) fprintf(fout,"%c","ACGTN"[(int)p->seq[j]]);
		else for (j = 0; j != p->full_len; ++j) fprintf(fout,"%c","TGCAN"[p->seq[p->full_len - 1 - j]]);
		fprintf(fout,"%c",'\t');
		if (p->qual) {
			if (p->strand) seq_reverse(p->len, p->qual, 0); // reverse quality
			err_fprintf(fout,"%s", p->qual);
		} else err_fprintf(fout,"*");

		if (bwa_rg_id[0]) err_fprintf(fout,"\tRG:Z:%s", bwa_rg_id);
		if (p->bc[0]) err_fprintf(fout,"\tBC:Z:%s", p->bc);
		if (p->clip_len < p->full_len) err_fprintf(fout,"\tXC:i:%d", p->clip_len);
		if (p->type != BWA_TYPE_NO_MATCH) {
			int i;
			// calculate XT tag
			XT = "NURM"[p->type];
			if (nn > 10) XT = 'N';
			// print tags
			err_fprintf(fout,"\tXT:A:%c\t%s:i:%d", XT, (mode & BWA_MODE_COMPREAD)? "NM" : "CM", p->nm);
			if (nn) err_fprintf(fout,"\tXN:i:%d", nn);
			if (mate) err_fprintf(fout,"\tSM:i:%d\tAM:i:%d", p->seQ, am);
			if (p->type != BWA_TYPE_MATESW) { // X0 and X1 are not available for this type of alignment
				err_fprintf(fout,"\tX0:i:%d", p->c1);
				if (p->c1 <= max_top2) err_fprintf(fout,"\tX1:i:%d", p->c2);
			}
			err_fprintf(fout,"\tXM:i:%d\tXO:i:%d\tXG:i:%d", p->n_mm, p->n_gapo, p->n_gapo+p->n_gape);
			if (p->md) err_fprintf(fout,"\tMD:Z:%s", p->md);
			// print multiple hits
			if (p->n_multi) {
				err_fprintf(fout,"\tXA:Z:");
				for (i = 0; i < p->n_multi; ++i) {
					bwt_multi1_t *q = p->multi + i;
					int k;
					j = pos_end_multi(q, p->len) - q->pos;
					nn = bns_cnt_ambi(bns, q->pos, j, &seqid);
					err_fprintf(fout,"%s,%c%d,", bns->anns[seqid].name, q->strand? '-' : '+',
						   (int)(q->pos - bns->anns[seqid].offset + 1));
					if (q->cigar) {
						for (k = 0; k < q->n_cigar; ++k)
							err_fprintf(fout,"%d%c", __cigar_len(q->cigar[k]), "MIDS"[__cigar_op(q->cigar[k])]);
					} else err_fprintf(fout,"%dM", p->len);
					err_fprintf(fout,",%d;", q->gap + q->mm);
				}
			}
		}
		fprintf(fout,"%c",'\n');
		...
		err_fprintf(fout,"%s\t%d\t*\t0\t0\t*\t*\t0\t0\t", p->name, flag);
		for (j = 0; j != p->len; ++j) fprintf(fout,"%c","ACGTN"[(int)s[j]]);
		fprintf(fout,"%c",'\t');
		if (p->qual) {
			if (p->strand) seq_reverse(p->len, p->qual, 0); // reverse quality
			err_fprintf(fout,"%s", p->qual);
