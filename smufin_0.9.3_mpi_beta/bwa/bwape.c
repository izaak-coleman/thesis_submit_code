...
void bwa_print_sam_SQ(const bntseq_t *bns, FILE* fout);

void fbwa_print_sam_PG(FILE* fout)
{
        fprintf(fout, "@PG\tID:bwa\tPN:bwa\tVN:%s\n", "0.7.4");
}
...
void my_bwa_sai2sam_pe_core(const char *prefix, char* sai_1_filename, char* sai_2_filename, char* fastq_1_filename, char* fastq_2_filename, pe_opt_t *popt, const char *rg_line, FILE* fout)
{
	extern bwa_seqio_t *bwa_open_reads(int mode, const char *fn_fa);
	int i, j, n_seqs, tot_seqs = 0;
	bwa_seq_t *seqs[2];
	...
	bns = bns_restore(prefix);
	srand48(bns->seed);
	fp_sa[0] = xopen(sai_1_filename, "r");
	fp_sa[1] = xopen(sai_2_filename, "r");
	g_hash = kh_init(b128);
	last_ii.avg = -1.0;

	fread(&opt, sizeof(gap_opt_t), 1, fp_sa[0]);
	ks[0] = bwa_open_reads(opt.mode, fastq_1_filename);
	opt0 = opt;
	fread(&opt, sizeof(gap_opt_t), 1, fp_sa[1]); // overwritten!
	ks[1] = bwa_open_reads(opt.mode, fastq_2_filename);
	{ // for Illumina alignment only
		if (popt->is_preload) {
			strcpy(str, prefix); strcat(str, ".bwt");  bwt = bwt_restore_bwt(str);
			strcpy(str, prefix); strcat(str, ".sa"); bwt_restore_sa(str, bwt);
	...
	// core loop
	bwa_print_sam_hdr(bns, rg_line, fout);
	fbwa_print_sam_PG(fout);
	while ((seqs[0] = bwa_read_seq(ks[0], 0x40000, &n_seqs, opt0.mode, opt0.trim_qual)) != 0) {
		int cnt_chg;
		isize_info_t ii;
		ubyte_t *pacseq;
		...
		for (i = 0; i < n_seqs; ++i) {
			...
			bwa_print_sam1(bns, p[0], p[1], opt.mode, opt.max_top2, fout);
			bwa_print_sam1(bns, p[1], p[0], opt.mode, opt.max_top2, fout);
		}
		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
...
*/
int my_bwa_sai2sam_pe(char* ref_file, char* sai_1_file, char* sai_2_file, char* fastq_1_file, char* fastq_2_file, char* sam_out_file)
{
	int c;
	pe_opt_t *popt;
	char *prefix, *rg_line = 0;
	...
	if ((prefix = bwa_idx_infer_prefix(ref_file)) == 0) {
		fprintf(stderr, "[%s] fail to locate the index\n", __func__);
		return 0;
	}

	FILE* fout = fopen(sam_out_file, "w");
	my_bwa_sai2sam_pe_core(prefix, sai_1_file, sai_2_file, fastq_1_file, fastq_2_file, popt, rg_line, fout);
	fclose(fout);
	free(prefix); free(popt);
	return 0;
}
