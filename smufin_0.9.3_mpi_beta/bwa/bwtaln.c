...
void my_bwa_aln_core(const char *prefix, const char *fn_fa, const gap_opt_t *opt, FILE* fout)
{
	int i, n_seqs, tot_seqs = 0;
	bwa_seq_t *seqs;
	bwa_seqio_t *ks;
	...
	// core loop
	err_fwrite(opt, sizeof(gap_opt_t), 1, fout);
	while ((seqs = bwa_read_seq(ks, 0x40000, &n_seqs, opt->mode, opt->trim_qual)) != 0) {
		tot_seqs += n_seqs;
		t = clock();
		...
		for (i = 0; i < n_seqs; ++i) {
			bwa_seq_t *p = seqs + i;
			err_fwrite(&p->n_aln, 4, 1, fout);
			if (p->n_aln) err_fwrite(p->aln, sizeof(bwt_aln1_t), p->n_aln, fout);
		}
		fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
...
int my_bwa_aln(char* ref_file, char* fastq_file, char* out_sai_file)
{
	int c, opte = -1;
	gap_opt_t *opt;
	char *prefix;
	...
	FILE* fout = fopen(out_sai_file, "wb");
	my_bwa_aln_core(prefix, fastq_file, opt, fout);
	fclose(fout);
