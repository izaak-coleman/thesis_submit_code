...
int bam_merge_main(FILE* stream_out, int n, char** mem_files, size_t* size_mem_files, int n_threads)
{
	int flag = 0;
	int level = -1;
	int by_qname = 0;
	...
	for (i = 0; i != n; ++i) {
		bam_header_t *hin;
		fp[i] = bam_fopen(fmemopen(mem_files[i], size_mem_files[i], "rb"), "r");
		if (fp[i] == 0) {
			int j;
			fprintf(stderr, "[bam_merge_core] fail to open file\n");
			for (j = 0; j < i; ++j) bam_close(fp[j]);
			free(fp); free(heap);
			// FIXME: possible memory leak
			return -1;
		...
		if (i == 0) { // the first BAM
			...
			for (j = 0; j < min_n_targets; ++j)
				if (strcmp(hout->target_name[j], hin->target_name[j]) != 0) {
					fprintf(stderr, "[bam_merge_core] different target sequence name: '%s' != '%s'\n",
							hout->target_name[j], hin->target_name[j]);
					return -1;
				}
	...
	if (hheaders) {
		...
		if (hheaders->n_targets > 0) {
			if (hout->n_targets != hheaders->n_targets) {
				fprintf(stderr, "[bam_merge_core] number of @SQ headers in differs from number of target sequences\n");
				if (!reg) return -1;
			}
			for (j = 0; j < hout->n_targets; ++j)
				if (strcmp(hout->target_name[j], hheaders->target_name[j]) != 0) {
					fprintf(stderr, "[bam_merge_core] @SQ header '%s' in differs from target sequence\n", hheaders->target_name[j]);
					if (!reg) return -1;
				}
		}
	...
	strcpy(mode, "w");
	if (level >= 0) sprintf(mode + 1, "%d", level < 9? level : 9);
	if ((fpout = bam_fopen(stream_out, mode)) == 0) {
		fprintf(stderr, "[%s] fail to create the output file.\n", __func__);
		return -1;
	}
	bam_header_write(fpout, hout);
	...
	while (heap->pos != HEAP_EMPTY) {
		...
		if ((j = bam_iter_read(fp[heap->i], iter[heap->i], b)) >= 0) {
			...
			free(heap->b->data); free(heap->b);
			heap->b = 0;
		} else fprintf(stderr, "[bam_merge_core] is truncated. Continue anyway.\n");
		ks_heapadjust(heap, 0, n, heap);
	}
...
static void write_fbuffer(FILE* stream_out, const char *mode, size_t l, bam1_p *buf, const bam_header_t *h, int n_threads)
{
	size_t i;
	bamFile fp;
	fp = bam_fopen(stream_out, mode);
	if (fp == 0) return;
	bam_header_write(fp, h);
	if (n_threads > 1) bgzf_mt(fp, n_threads, 256);
	for (i = 0; i < l; ++i)
...
void bam_sort_core_fext(int is_by_qname, char* bam_mem, size_t size_bam_mem, FILE* stream_out, const char *prefix, size_t _max_mem, int is_stdout, int n_threads, int level, int full_path)
{
	int ret, i, n_files = 0;
	size_t mem, max_k, k, max_mem;
	bam_header_t *header;
	...
	fp = bam_fopen(fmemopen(bam_mem, size_bam_mem, "rb"), "r");
	if (fp == 0) {
		fprintf(stderr, "[bam_sort_core] fail to open file \n");
		return;
	}
	header = bam_header_read(fp);
	if (is_by_qname) change_SO(header, "queryname");
	...
	if (n_files == 0) { // a single block
		...
		if (level >= 0) sprintf(mode + 1, "%d", level < 9? level : 9);
		ks_mergesort(sort, k, buf, 0);
		write_fbuffer(stream_out, mode, k, buf, header, n_threads);
	} 
	/*
	else { // then merge
		char **fns;
...
int bam_sort(char* mem_bam, size_t size_mem_bam, FILE* stream_out)
{
	size_t max_mem = 2000<<20; // 2000MB
	int c, is_by_qname = 0, is_stdout = 0, n_threads = 1, level = -1, full_path = 1;
	/*
	...
	*/
	char sfull_path[1000];

	sprintf(sfull_path, "/dev/shm/smufin.%d", time(0));
	bam_sort_core_fext(is_by_qname, mem_bam, size_mem_bam, stream_out, sfull_path, max_mem, is_stdout, n_threads, level, full_path);
	return 0;
}
