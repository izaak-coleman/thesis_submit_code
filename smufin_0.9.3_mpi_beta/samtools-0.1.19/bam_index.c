...
bam_index_t *bam_index_load_file(FILE* hfile)
{
        return bam_index_load_core(hfile);
}
...
int bam_findex(char* mem_bam, size_t size_mem_bam, FILE* stream_out)
{
	bamFile fp;
	bam_index_t *idx;
	if ((fp = bam_fopen(fmemopen(mem_bam, size_mem_bam, "rb"), "r")) == 0) {
		fprintf(stderr, "[bam_index_build2] fail to open the BAM file.\n");
		return -1;
	}
	idx = bam_index_core(fp);
	...
	bam_index_save(idx, stream_out);
	bam_index_destroy(idx);
	fclose(stream_out);
	return 0;
}

int block_findex(char* mem, size_t size_mem, FILE* stream_out)
{
	return bam_findex(mem, size_mem, stream_out);
}
