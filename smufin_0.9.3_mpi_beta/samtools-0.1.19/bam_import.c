...
restart:
	...
	b->l_aux = doff - doff0;
	b->data_len = doff;
	if (!valid) goto restart;
	if (bam_no_B) bam_remove_B(b);
	return z;
}

tamFile sam_memopen(char* mem, size_t size_mem)
{
	tamFile fp;
	FILE* gzfp = fmemopen(mem, size_mem, "r");
	if (gzfp == 0) return 0;
...
{
	tamFile fp;
	FILE* gzfp = (strcmp(fn, "-") == 0)? stdin : fopen(fn, "rb");
	if (gzfp == 0) return 0;
	fp = (tamFile)calloc(1, sizeof(struct __tamFile_t));
	fp->str = (kstring_t*)calloc(1, sizeof(kstring_t));
	fp->fp = gzfp;
...
void sam_close(tamFile fp)
{
	if (fp) {
		ks_destroy(fp->ks);
		if (fp->fp != stdin && fp->fp != stdout) fclose(fp->fp);
