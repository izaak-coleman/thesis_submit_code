...
samfile_t *samfopen(FILE* stream, const void *aux)
{
	samfile_t *fp;
	fp = (samfile_t*)calloc(1, sizeof(samfile_t));
		fp->type |= TYPE_READ;
			fp->type |= TYPE_BAM;
			fp->x.bam = bam_fopen(stream, "rb");
...
samfile_t *blockfopen(FILE* stream, const void *aux)
{
	return samfopen(stream, aux);
}
...
samfile_t *samopen(const char *fn, const char *mode, const void *aux)
{
	...
	if (strchr(mode, 'r')) { // read
		...
		if (strchr(mode, 'b')) { // binary
			...
			} else if (bam_verbose >= 2) fprintf(stderr, "");
		}
...
samfile_t *sammemopen_read(char* mem, size_t size_mem, const void *aux)
{
	samfile_t *fp;
	fp = (samfile_t*)calloc(1, sizeof(samfile_t));
		fp->type |= TYPE_READ;
			fp->x.tamr = sam_memopen(mem, size_mem);
			if (fp->x.tamr == 0) goto open_err_ret;
			fp->header = sam_header_read(fp->x.tamr);
			fprintf(stderr, "num_targets: %d\n", fp->header->n_targets);
			if (fp->header->n_targets == 0) { // no @SQ fields
...
samfile_t *sammemopen_write(FILE* fout, const char *mode, const void *aux)
{
	samfile_t *fp;
	fp = (samfile_t*)calloc(1, sizeof(samfile_t));
		fp->header = bam_header_dup((const bam_header_t*)aux);
			...
			bmode[0] = 'w'; bmode[1] = compress_level < 0? 0 : compress_level + '0'; bmode[2] = 0;
			fp->type |= TYPE_BAM;
			fp->x.bam = bgzf_fopen(fout, bmode);
			if (fp->x.bam == 0) goto open_err_ret;
			bam_header_write(fp->x.bam, fp->header);
...
void samclose(samfile_t *fp)
{
	...
	else if (fp->x.tamw != stdin && fp->x.tamw != stdout)fclose(fp->x.tamw);
	free(fp);
}

void blockclose(samfile_t *fp)
{
	samclose(fp);
}
